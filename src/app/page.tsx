...
export default function Page() {
  ...
  // if there're more data, we could use debouncedInput to optimize performance
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setTimeout(() => {
        setCheckInStatus(JSON.parse(stored));
      }, 0);
    }
  ...
  // generate search options based on jobs data, including job title, publicId and operator names
  // for the same jobId, generate different options with different type, so when user select one option, we can easily identify which job it belongs to and what type it is
  const searchOptions: SearchOption[] = jobs.flatMap((job) => {
    const operatorOptions: SearchOption[] = job.operators.map((op) => ({
  ...
  const fetchAutoCompleteOptions = (event: SyntheticEvent, value: string) => {
    setInputValue(value);
  ...
  const getRowData = (operators: Operator[]) =>
    operators?.map((op) => ({
      id: op.id,
      ...
      {!loading && !errorMessage && jobs.length > 0 && (
        <>
          ...
          {filteredJobs.map((job) => {
            ...
            const rowData = getRowData(job?.operators || []);
